...
class Simulator:
    def __init__(self, infectious_rate=25, uninfectious_rate=10, sampling_rate=1, sampling_probability=None,
                 sites_number=0, mutation_rate=None, mutation_probabilities=None, populations_number=1,
                 population_size=1000000, contact_density=1.0, total_migration_probability=0.0, lockdown=None,
                 sampling_multiplier=None, immunity_type=None, susceptibility=None, total_immunity_transition=None):
        # Get rates
        if infectious_rate >= 0 and uninfectious_rate >= 0 and sampling_rate >= 0:
            ...
            if sampling_probability != None:
                if 0 <= sampling_probability <= 1:
                    self.D_rate = [float(uninfectious_rate) * (1 - sampling_probability) for _ in
                                   range(4 ** sites_number)]
        ...
        if sites_number != 0:
            if mutation_rate != None and mutation_probabilities != None:
                if 0 <= mutation_rate and len(mutation_probabilities) == 3 and mutation_probabilities[0] >= 0 and \
                        mutation_probabilities[1] >= 0 and mutation_probabilities[2] >= 0:
                    self.mutations = [[[mutation_rate] + mutation_probabilities for _ in range(sites_number)] for _ in
                                      range(4 ** sites_number)]
                else:
                    self.Error(
                        "\"mutation_rate\" should be more 0 and less 1, lenght of \"mutation_probabilities\" should be equal 3 and each element should be more or equal 0!")
            else:
                self.Error(
                    "Mutations model consist of \"sites_number\", \"mutation_rate\" and \"mutation_probabilities\"!")
        ...
        if population_size >= 1 and contact_density >= 0 and 0 <= total_migration_probability <= 1:
            ...
            self.Error(
                "\"population_size\" should be more 1, \"contact_dencity\" should be more 0 and \"total_migration_probability\" should be more or equal 0 and less or equal 1!")
        ...
        if lockdown != None:
            if lockdown[0] >= 0 and 0 <= lockdown[1] <= 100 and 0 <= lockdown[2] <= 100 and len(lockdown) == 3:
                ...
                self.Error(
                    "lenght of \"lockdown\" should be equal 3, first element should be more or equal 0 and second and third elements should be more or equal 0 and less or equal 100!")
    ...
    def initialize(self, _seed=None):
        ...
        self.simulation = BirthDeathModel(self.B_rate, self.D_rate, self.S_rate, self.mutations,
                                          populationModel=[self.populations, self.migration], susceptible=self.susc,
                                          suscepTransition=self.suscTrans, lockdownModel=self.lockdowns,
                                          samplingMultiplier=self.samplingMultiplier, rndseed=int(_seed))
